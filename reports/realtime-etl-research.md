...
Expanded top-five combination reports with corrected live UI evidence are in [reports/combinations](combinations/README.md). StreamPark and Dinky screenshots there are logged-in dashboard captures with configured jobs visible; SeaTunnel now distinguishes separate SeaTunnel Web job-authoring capability from the locally proven Engine UI monitoring screenshot; Camel is explicitly labeled as runtime endpoint evidence because no logged-in ETL workbench exists in this proof.
...
| Apache InLong | Official compose files in [local-setup/inlong](/Users/oka/Documents/etl-research/local-setup/inlong), phase-2 K8s manifest in [inlong.yaml](/Users/oka/Documents/etl-research/local-setup/phase2-k8s/inlong.yaml) | Started the local dashboard, manager, and MySQL control-plane slice from official compose files; dashboard returned HTTP 200 and rendered the data ingestion UI. The manager container started Tomcat on 8083 but `curl` returned `Empty reply from server` and the dashboard displayed `http error` toasts under `linux/amd64` emulation. The stricter phase-2 Kubernetes proof therefore validates the InLong Transform SDK path directly: Kafka-compatible source topic -> InLong SQL JSON filter/enrich -> Kafka-compatible JSON sink topic |
...
| Dinky | Docker container `etl-research-dinky` | Standalone server is up on `http://localhost:18888`; admin login works; Data Studio shows configured Flink SQL task `wallet_payment_flink_sql_filter` with Kafka/Redpanda source, transform, and sink SQL |
...
| 4 | Apache InLong | Complete | Kafka/Redpanda | `phase2-inlong-wallet-events-<run-id>` -> Java pod using Apache InLong Transform SDK SQL over JSON filter/enrich -> `phase2-inlong-wallet-filtered-<run-id>` JSON sink topic | [pods](../local-setup/phase2-k8s-proofs/04-inlong-pods-running.txt), [summary](../local-setup/phase2-k8s-proofs/04-inlong-summary.txt), [source](../local-setup/phase2-k8s-proofs/04-inlong-source-topic.txt), [sink](../local-setup/phase2-k8s-proofs/04-inlong-sink-topic.txt) | ![InLong K8s proof](../screenshots/phase2-k8s/04-inlong-k8s-proof.png) |
...
| 8 | Apache StreamPipes | Complete | StreamPipes internal NATS | Machine Data Simulator compact adapter -> compact adapter field rename `temperature` to `amount` -> StreamPipes-created Data Lake persist pipeline | [pods](../local-setup/phase2-k8s-proofs/08-streampipes-pods-running.txt), [summary](../local-setup/phase2-k8s-proofs/08-streampipes-summary.txt), [count](../local-setup/phase2-k8s-proofs/08-streampipes-datalake-count.json), [sample](../local-setup/phase2-k8s-proofs/08-streampipes-datalake-sample.json) | ![StreamPipes K8s proof](../screenshots/phase2-k8s/08-streampipes-k8s-proof.png) |
...
| 4 | Apache InLong | 1,491 | High: manager/API model for data streams and sync jobs | High | Java/Flink ecosystem | Good real-time integration architecture; RocketMQ path should be version-validated | [runtime proof](../screenshots/inlong-runtime-proof.png), [dashboard screenshot](../screenshots/inlong-dashboard.png), [K8s proof](../screenshots/phase2-k8s/04-inlong-k8s-proof.png), [compose](../local-setup/inlong/docker-compose.yml), [card](../screenshots/candidates/04-apache-inlong.png) | Good governance platform and Transform SDK is usable from Java; validate Manager API health before choosing it as the primary platform |
